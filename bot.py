...
def check_varibles():
    ...
    except: SUMM_BUY_TRAFFIC = 150  # 💰 цена за СБРОС трафика (укажи свою)

    global TRAFFIC_TOPUP_GB
    try: TRAFFIC_TOPUP_GB
    except: TRAFFIC_TOPUP_GB = 500   # 📶 сколько ГБ докупаем за раз
    ...
    async def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                ...
                logger.warning(f'Не удалось выполнить запрос (DB.{func.__name__}): {e}')
                if i > 3:
                    raise e
...
async def db_connect(db_file=None):
    ...
    if asyncpg is None:
        raise RuntimeError("asyncpg не установлен для работы с Postgres")
...
async def dump_postgres_sql(file_path, tables=None):
    ...
    try:
        ...
        logger.warning(f'🛑Не удалось сделать дамп Postgres: {e}')
        return False
    ...
    try:
        ...
        if 'supervisorctl restart bot' in command:
            if DISABLE_SUPERVISOR:
                logger.warning('⚠️Перезапуск через Docker (supervisor отключен)')
                os._exit(1)
...
class UserBot:
    def __init__(self, id_Telegram):
        try:
            ...
            self.donate_text = ''
            self.tarif_select = 1
            self.isPayTraffic = False          # оплата именно за докупку трафика
            self.traffic_add_gb = 0           # сколько ГБ докупаем
    ...
    async def EXECUTE(self, query='', args=(), res=False):
        try:
            ...
            if stripped.startswith("CREATE") or stripped.startswith("ALTER"):
                logger.warning(f'🛑DB.EXECUTE schema error: {e}\n  query: {query[:200]}')
            return False
...
#region Глобальный кэш токенов Marzban (для производительности)
# Ключ = domain, значение = {'token': str, 'expires': datetime}
_marzban_token_cache = {}
_marzban_token_cache_lock = asyncio.Lock()
# Кэш недоступных серверов (чтобы не спамить запросами)
...
async def get_marzban_token(domain: str, password: str, retry: int = 2) -> str:
    """Получить токен из кэша или авторизоваться и закэшировать"""
    global _marzban_token_cache, _marzban_unavailable_servers
    
    cache_key = f"{domain}:{password}"
    now = datetime.now()
    
    # Проверяем, не был ли сервер недавно недоступен (кэшируем на 5 минут)
    if cache_key in _marzban_unavailable_servers:
        unavail_time = _marzban_unavailable_servers[cache_key]
        if (now - unavail_time).total_seconds() < 300:  # 5 минут
            logger.debug(f'⏸️ Сервер {domain} недоступен, пропускаем (кэш недоступности)')
            return None
    ...
    async with _marzban_token_cache_lock:
        if cache_key in _marzban_token_cache:
            ...
            # Токен живёт 55 минут (Marzban даёт на 1 час)
            if cached['expires'] > now:
                return cached['token']
    
    # Авторизуемся асинхронно с retry
            ...
            async with aiohttp.ClientSession(timeout=get_timeount(10)) as session:
                async with session.post(url, headers=headers, data=data, ssl=False) as response:
                    if response.status != 200:
                        logger.warning(f'🛑get_marzban_token: {domain} вернул статус {response.status}')
                        continue
                    result = await response.json()
                    token = result.get('access_token')
                    if not token:
                        logger.warning(f'🛑get_marzban_token: {domain} не вернул access_token')
                        continue
                    
                    # Кэшируем на 55 минут
                    async with _marzban_token_cache_lock:
                    ...
                    # Убираем из списка недоступных
                    _marzban_unavailable_servers.pop(cache_key, None)
                    return token
        except aiohttp.ClientConnectorError as e:
            logger.warning(f'🛑get_marzban_token: не удалось подключиться к {domain} (попытка {attempt+1}/{retry}): {e}')
            if attempt < retry - 1:
                await asyncio.sleep(1)
        except Exception as e:
            logger.warning(f'🛑get_marzban_token ошибка для {domain} (попытка {attempt+1}/{retry}): {e}')
            if attempt < retry - 1:
                await asyncio.sleep(1)
    
    # Сервер недоступен — запоминаем на 5 минут
    _marzban_unavailable_servers[cache_key] = now
    logger.warning(f'🛑get_marzban_token: сервер {domain} недоступен, кэшируем на 5 мин')
    return None
#endregion
...
class KEYS_ACTIONS:
    async def activateKey(protocol, conf_name, ip_server=None, user_id=None, days=1):
        ...
        if not ip_server:
            ...
            if not ip_server:
                logger.warning(f'🛑activateKey: ip_server не найден для {conf_name}')
                return False
    ...
    async def deactivateKey(protocol, conf_name, ip_server=None, date=None, CountDaysBuy=None, user_id=None):
        ...
        if not ip_server:
            ip_server = await DB.get_ip_server_by_key_name(conf_name)
            if not ip_server:
                logger.warning(f'🛑deactivateKey: ip_server не найден для {conf_name}')
                return False
...
class MARZBAN:
    ...
    ОПТИМИЗИРОВАНО: Используется aiohttp вместо requests для асинхронных запросов
    """
    ...
    def __init__(self, domain=None, password=None, ip=None):
        ...
        self.session = None  # Оставляем для обратной совместимости (SSH методы)
    ...
    async def _get_auth_headers(self) -> dict | None:
        """Получить headers с токеном авторизации (асинхронно, с кэшированием)
        Возвращает None, если сервер недоступен"""
    ...
    def _connect_api(self):
        """DEPRECATED: Оставлен для обратной совместимости SSH-методов. Используйте _get_auth_headers()"""
        if not self.session:
    ...
    async def _get_key_async(self, key):
        """Асинхронное получение данных ключа через aiohttp"""
        try:
            headers = await self._get_auth_headers()
            if not headers:
                logger.warning(f'🛑_get_key_async: сервер {self.domain} недоступен, пропускаем ключ {key}')
                return None
    ...
    def _get_key(self, key):
        """DEPRECATED: Синхронная версия, оставлена для обратной совместимости"""
        self._connect_api()
    ...
    async def _get_link_async(self, key, response=None):
        """Асинхронная версия получения ссылки"""
        if response:
            data_key = response
        else:
        ...
        try:
            ...
            if domain != '' and password != '' and user_id and location != '':
                ...
                commands = [
                    ...
                    f"echo -e '\nSUB_UPDATE_INTERVAL = 1\nUVICORN_PORT = 1443\nUVICORN_SSL_CERTFILE = \"/var/lib/marzban/certs/fullchain.pem\"\nUVICORN_SSL_KEYFILE = \"/var/lib/marzban/certs/key.pem\"\nXRAY_SUBSCRIPTION_URL_PREFIX = https://{domain}:1443\nXRAY_JSON = \"/var/lib/marzban/xray_config.json\"\nSQLALCHEMY_DATABASE_URL = \"sqlite:////var/lib/marzban/db.sqlite3\"\nACTIVE_STATUS_TEXT = \"Активна\"\nEXPIRED_STATUS_TEXT = \"Истек\"\nDISABLED_STATUS_TEXT = \"Отключено\"\nONHOLD_STATUS_TEXT = \"На удержании\"' >> /opt/marzban/.env",
                    'marzban restart',
                    'marzban up',
    ...
    async def create_new_key(self, key, date, days, is_premium: bool = False):
        ...
        try:
            ...
            # Лимиты трафика: 500 ГБ в месяц
            data_limit = 500 * 1024 * 1024 * 1024  # 500 ГБ в байтах
    ...
    async def update_status_key(self, key, status=True):
        ...
        try:
            ...
            if not data_db:
                logger.warning(f'🛑update_status_key: ключ {key} не найден в БД')
                return None
            ...
            # Асинхронно получаем данные ключа из Marzban
            data_key = await self._get_key_async(key)
            if not data_key or 'proxies' not in data_key:
                logger.warning(f'🛑update_status_key: не удалось получить данные ключа {key} из Marzban')
                return None
    ...
    async def update_status_1key(self, key, status=True, is_premium: bool | None = None):
        ...
        try:
            ...
            if not data_db:
                logger.warning(f'🛑update_status_1key: ключ {key} не найден в БД')
                return None
            ...
            # Если is_premium не передали — определяем по серверу
            ...
            # Асинхронно получаем данные ключа
            data_key = await self._get_key_async(key)
            if not data_key or 'proxies' not in data_key:
                logger.warning(f'🛑update_status_1key: не удалось получить данные ключа {key} из Marzban')
                return None
            ...
            headers = await self._get_auth_headers()
            if not headers:
                logger.warning(f'🛑update_status_1key: сервер {self.domain} недоступен, пропускаем ключ {key}')
                return None
            ...
            async with aiohttp.ClientSession(timeout=get_timeount(10)) as session:
                async with session.put(url, headers=headers, json=payload, ssl=False) as response:
                    if response.status not in (200, 201):
                        body = await response.text()
                        logger.warning(f'🛑update_status_1key: {key} вернул {response.status}: {body[:200]}')
                        return None
                    result = await response.json()
                    logger.debug(f"✅Изменили статус ключа {key}: {result}")
                    return result
            ...
            logger.warning(f"🛑Ошибка в update_status_1key (key={key}, status={status}, is_premium={is_premium}): {e}")
            return None            
    ...
    async def reset_limit_and_usage(self, key: str):
        """
        ОПТИМИЗИРОВАНО: Полностью асинхронная версия с aiohttp
        Сбрасывает лимит и потребление для пользователя key.
        Заточено под премиум-локации:
         - data_limit = 500 ГБ
         - data_limit_reset_strategy = "month"
         - usage обнуляется через /user/{key}/reset
        """
        try:
            headers = await self._get_auth_headers()
            if not headers:
                logger.warning(f'🛑reset_limit_and_usage: сервер {self.domain} недоступен, пропускаем ключ {key}')
                return

            # Базовый лимит для премиум-ключей
            payload = {
                "data_limit": 500 * 1024 * 1024 * 1024,   # 500 ГБ
                "data_limit_reset_strategy": "month",
            }
    ...
    async def add_traffic(self, key, add_gb: int) -> bool:
        """
        ОПТИМИЗИРОВАНО: Полностью асинхронная версия с aiohttp
        Добавляет к текущему лимиту трафика пользователя add_gb ГБ
        """
        try:
            # Асинхронно получаем текущие данные ключа
            data_key = await self._get_key_async(key)
            if not data_key:
                logger.warning(f'🛑add_traffic: не найден пользователь {key} в Marzban')
                return False

            current_limit = int(data_key.get('data_limit') or 0)
            add_bytes = int(add_gb * 1024 * 1024 * 1024)  # ГБ -> байты
            ...
            headers = await self._get_auth_headers()
            if not headers:
                logger.warning(f'🛑add_traffic: сервер {self.domain} недоступен, пропускаем ключ {key}')
                return False
            ...
            async with aiohttp.ClientSession(timeout=get_timeount(10)) as session:
                async with session.put(url, headers=headers, json=payload, ssl=False) as response:
                    ...
                    try:
                        result = await response.json()
                        logger.debug(f'✅add_traffic: обновили лимит для {key} до {new_limit} байт: {result}')
                    except:
                        logger.debug(f'✅add_traffic: обновили лимит для {key} до {new_limit} байт, статус {status_code}')

                    return status_code in (200, 201)
                    
        except Exception as e:
            logger.warning(f'🛑Ошибка в add_traffic (key={key}, add_gb={add_gb}): {e}')
            return False
    ...
    async def create_test_keys_bulk(self, count: int = 300, days: int = 30, user_id: int = None, ip_server: str = None, expire_today: bool = False):
        """
        Тестовая функция: массовое создание ключей для проверки производительности.
        Симулирует покупку ключей обычными пользователями.
        
        count - количество ключей
        days - срок действия (если expire_today=True, то ключи истекут сегодня)
        user_id - ID пользователя, кому привязать ключи
        ip_server - IP сервера Marzban
        expire_today - если True, ключи создаются как "истёкшие" для теста отключения
        """
        import random
        import string
        ...
        # Если expire_today=True, ставим дату создания "days назад", чтобы ключ истёк сегодня
        if expire_today:
            date_create = (datetime.now() - timedelta(days=days)).strftime('%Y_%m_%d')
            days_in_marzban = 0  # В Marzban ключ уже истёк
        else:
        ...
        async def create_one_key(index):
            ...
            async with semaphore:
                try:
                    # Генерируем уникальное имя ключа
                    ...
                    if result:
                        # Записываем ключ в БД бота как ОБЫЧНЫЙ пользовательский ключ
                        await DB.add_qr_key(
                            user_id=user_id,
                            vpn_key=key_name,
                            date=date_create,  # Дата "покупки" (для расчёта срока)
                            os='Android',
                            isAdminKey=0,  # ← Обычный ключ (НЕ админский)
                            ip=ip_server,
                            days=days,       # Срок на который "куплен"
                            summ=149,        # Симуляция оплаты
                            bill_id='test_bulk',
                            protocol='vless',
                            isChangeProtocol=False,
                            keys_data=result,  # URL подписки
                            podpiska=-1
                        )
                        created += 1
                        if created % 50 == 0:
                            logger.debug(f'✅ Создано {created}/{count} ключей...')
                    else:
                        failed += 1
                except Exception as e:
                    failed += 1
                    logger.warning(f'🛑 Ошибка создания ключа #{index}: {e}')
        
        # Отправляем сообщение о начале
        mode_text = "⚠️ ИСТЕКАЮТ СЕГОДНЯ (для теста отключения)" if expire_today else f"на {days} дней"
        if user_id:
            await send_message(user_id, 
                f'🔄 Начинаю создание {count} тестовых ключей {mode_text}...\n\n'
                f'• Семафор: 30 параллельных запросов\n'
                f'• Привязка к: {user_id}\n'
                f'• isAdminKey: 0 (обычные ключи)\n\n'
                f'После создания запусти /test_update для проверки отключения'
            )
        ...
        result_text = (
            ...
            f'📊 Результаты:\n'
            f'• Создано: {created}/{count}\n'
            f'• Ошибок: {failed}\n'
            f'• Время: {elapsed:.1f} сек\n'
            f'• Скорость: {count/elapsed:.1f} ключей/сек\n'
            f'• Привязано к: {user_id}\n'
            f'• Режим: {"ИСТЕКАЮТ СЕГОДНЯ" if expire_today else "обычные"}\n\n'
            f'🔍 Теперь запусти /test_update для проверки'
        )
        ...
        try:
            if self.con:
                ...
                try:
                    ...
                    logger.debug(f'Не удалось получить дату из БД: {e}, fallback на параметры days/date')
                    try:
                        if date:
                            date_start = datetime.strptime(date, '%Y_%m_%d')
                        else:
                        ...
                        logger.warning(f'Ошибка вычисления expiryTime: {e2}')
                        expiry_timestamp_ms = 0
                if isUpdate:
                    # Получаем текущий flow
...
async def update_bot():
    ...
    if await DB.EXECUTE('select * from Wallets', res=True) is None:
        # добавить текущий способ оплаты из файла .env
        if ACCESS_TOKEN != '':
...
async def test_cron():
    if DISABLE_CRON:
        logger.debug('⚠️Cron отключен (docker)')
        return True
    ...
    try:
        ...
        if len(keys_data) > 0:
            ...
            for item in keys_data:
                ...
                name_key_for_but = f'🔑{name_vpn_key}{count_days_to_off_text}'
                ...
                if payment_id and dop_text == ':download':
                    ...
                    text_galochka = '✅' if isActive else '☑️'
            ...
            if keys_yes:
                ...
                # нет активных ключей, показываем кнопки "Подключиться" и "Назад"
...
async def check_keys_all():
    try:
        ...
        async def check_key(line, semaphore):
            ...
            # Семафор 1 = строго поочерёдные запросы к серверу (без перегрузки API панели)
                ...
                try:
                    ...
                    try: date_start = datetime.strptime(date_key, '%Y_%m_%d')
                        ...
                        try:
                            ...
                            logger.warning(f'🛑check_key: некорректный формат даты для ключа {vpn_key}: {date_key}')
                            return
...
async def send_twohour_notifications(next_check_time: datetime):
    """
    next_check_time — момент, когда будет основная проверка/отключение (HOUR_CHECK).
    Здесь мы смотрим, какие ключи к этому времени уже подойдут к отключению,
    и шлём им предупреждение за 2 часа с кнопками оплаты.
    """
    try:
        lines = await DB.get_qr_key_All()
        if not lines:
        ...
        for line in lines:
            ...
            isActive    = bool(line[6])
            protocol    = line[7]
            # line[8], line[9] — isChangeProtocol / keys_data
            payment_id  = line[10]
            RebillId    = line[11]
            Podpiska    = int(line[12])
            date_time   = line[13]
            ...
            # админские и выключенные конфиги не трогаем
            if isAdminKey or not isActive:
                continue

            # дата окончания ключа — так же, как в check_keys_all / check_key
            try:
                date_start = datetime.strptime(date_key, '%Y_%m_%d')
            except Exception:
                continue
            ...
            # считаем days_for_close_period на момент будущего check'а
            days_for_close_period = (date_end - next_check_time).days + 1

            # в check_key отключение идёт при days_for_close_period < 1
            if days_for_close_period < 1:
                # ======= ВОТ ТУТ МЕНЯЕМ СХЕМУ ОТПРАВКИ СООБЩЕНИЯ =======
                try:
                    # если есть автосписание — можно сделать отдельный текст
                    if payment_id != '' and AUTO_PAY_YKASSA:
                        user = await user_get(user_id)
                        # если хочешь отдельную фразу — добавь tx_twohour_auto в lang.yml
                        # text_send = user.lang.get('tx_twohour_auto')
                        # если не хочешь заморачиваться — можно использовать тот же tx_twohour
                        text_send = user.lang.get('tx_twohour')
                        await send_message(user_id, text_send, no_log=True)
                    else:
                        # как в check_keys_all: готовим продление + кнопки
                        user = await user_get(user_id)
                        user.isProdleniye = vpn_key

                        text_send = user.lang.get('tx_twohour')
                        # внизу добавляем блок с предложением оплатить
                        text_send += '\n\n' + user.lang.get('tx_prodlt')

                        # ИМЕННО buy_message рисует клавиатуру с оплатой
                        await buy_message(user_id=user_id, text_send=text_send)

                    logger.debug(f'✉️ Отправили 2-часовое уведомление пользователю {user_id} по ключу {vpn_key}')
                except:
                    await Print_Error()
                # =======================================================
...
async def twohour_notify_loop():
    """
    Постоянный цикл:
    - находит ближайший запуск основного check'а по HOUR_CHECK
    - за 2 часа до него будитcя и вызывает send_twohour_notifications()
    """
    ...
    while True:
        try:
            ...
            # 1. Ближайший "основной" check по HOUR_CHECK
            #    (мы считаем, что check_keys_all гоняется раз в день в HOUR_CHECK по крону)
            next_check = datetime(now.year, now.month, now.day, HOUR_CHECK, 0, 0)

            # если сегодня HOUR_CHECK уже прошёл — берём завтрашний
            if next_check <= now:
                next_check += timedelta(days=1)

            # 2. Время, когда надо слать 2-часовые предупреждения
            notify_time = next_check - timedelta(hours=2)

            # если мы уже позже, чем notify_time (например, бот перезапустился в 22:30 при HOUR_CHECK=23),
            # то смещаемся на следующий день
            if notify_time <= now:
                next_check += timedelta(days=1)
                notify_time = next_check - timedelta(hours=2)

            sleep_seconds = (notify_time - now).total_seconds()
            logger.debug(f'⏰ 2-hour notify: спим до {notify_time}, потом шлём уведомления перед {next_check}')

            await asyncio.sleep(sleep_seconds)

            # Проснулись — шлём уведомления для ключей,
            # которые отключатся в next_check
            await send_twohour_notifications(next_check)

        except Exception:
            await Print_Error()
            # на всякий случай, чтобы не устроить быстрый бесконечный цикл при ошибке
...
async def add_days(user_id, conf_name, day=COUNT_DAYS_REF, promo='', silent=False):
    try:
        ...
        if not silent:
            if not IS_OTCHET:
                await send_admins(user_id, f'✅Ключ продлен', f'{promo_text}, <code>{ip_server}</code>')
...
def _split_for_caption(text: str, limit: int = CAPTION_LIMIT):
    """Возвращает (caption, rest). caption <= limit."""
    if text is None:
        return "", ""
    if len(text) <= limit:
        return text, ""
...
async def send_start_message(message, priglacili: bool = False):
    try:
        ...
        _tx_start += (f'\n<u>Ваш 🆔:</u> <code>{user_id}</code>\n'
                      f'<a href="https://t.me/evo_vpn_bot?start=">Ваши ключи :</a>\n')
        ...
        if keys:
            for idx, key in enumerate(keys, 1):
                ...
                _tx_start += f'<pre>{name} {status_emoji} | {days_str}, до {date_to}</pre>\n'
        else:
            _tx_start += "\nУ вас нет активных ключей.😢"

        # ---------- СНАЧАЛА ОТПРАВЛЯЕМ ФОТО ОТДЕЛЬНЫМ СООБЩЕНИЕМ ----------
        try:
            await bot.send_photo(chat_id=user_id, photo=WELCOME_PHOTO_FILE_ID)
        except BotBlocked:
            # пользователь заблокировал бота — ничего критичного, просто выходим
            return
        except WrongFileIdentifier:
            # неверный file_id/URL — заглушка, пропускаем фото
            pass
        except TimeoutError:
            # телега подвисла — просто не шлём картинку
            logger.warning(f"Timeout при отправке приветственного фото пользователю {user_id}")
        except Exception:
            # другие реальные ошибки уже отправляем в твой общий логгер
            await Print_Error()

        # ---------- ТЕПЕРЬ ОТПРАВЛЯЕМ ТЕКСТ И КНОПКИ ----------
        if INLINE_MODE:
            # В ИНЛАЙН-режиме используем клавиатуру user.klav_start
            # Сначала (если есть) приглашение…
            if invitation_text:
                try:
            ...
            # …затем основной блок со статусом и кнопками
            try:
                await send_message(user_id, _tx_start, reply_markup=user.klav_start, parse_mode='HTML')
            except:
                # если упало из-за parse_mode — повторяем без него
                try:
            ...
            # НЕ-инлайн: отдельное приглашение (если есть) + основной текст с инлайн-кнопками klava
            if invitation_text:
                try:
                    await send_message(user_id, invitation_text, reply_markup=user.klav_start)
                except:
            ...
            # Основной текст со "служебными" кнопками (тест-ключ/подключиться)
            try:
...
async def help_messages(message):
    try:
        ...
        # Сценарий:
        # 1) пользователь нажимает кнопку Android TV -> bot_status=90
        # 2) присылает UID (5 символов) -> бот отправляет user.key_url в Happ
        if user.bot_status == 90:
            # Таймаут ожидания UID (90 секунд)
            ...
            if ts and (time.time() - ts > 90):
                ...
                await send_message(
                    user_id,
                    f'⌛️ Время ожидания UID истекло (90 сек).\n\n{text_back}',
            ...
            if not re.match(r'^[A-Z0-9]{5}$', uid):
                await send_message(user_id, '❌ UID должен быть из 5 символов (A-Z, 0-9). Пришли код с экрана ТВ ещё раз.')
                return

            if not getattr(user, 'key_url', ''):
                user.bot_status = 0
                await send_message(user_id, '❌ Не нашёл текущий ключ. Сначала получи/создай ключ, потом снова нажми Android TV.')
                return
            ...
            try:
                ...
                if r.status_code // 100 == 2:
                    ...
                    klava_tv_back.add(InlineKeyboardButton(text='⏪ Назад', callback_data='tv:back'))
                    await send_message(
                        user_id,
                        '✅ Запрос отправлен в Happ. Если UID был актуальный — подписка появится в приложении. Если не появилось, попробуй ещё раз с новым UID.',
                        reply_markup=klava_tv_back
                    )
                else:
                    await send_message(user_id, f'❌ Ошибка Happ ({r.status_code}): {r.text[:300]}\n\nПроверь, что на ТВ открыт Web Import и UID актуальный.')
            except Exception as e:
                await send_message(user_id, f'❌ Не удалось отправить на Happ: {e}')
            return
        user.isAutoCheckOn = False
        user.isPayChangeProtocol = False
        user.isPayChangeLocations = False
...
async def new_key(user_id, day=30, is_Admin=0, promo='', help_message=False, summ=0, bill_id='', protocol=PR_DEFAULT, date=None, ip_server=None, silent=False, isChangeLocation=False, RebillId='', Podpiska=-1, summ_tarif=-1):
    ...
    try:
        ...
        logger.debug(
            f'Параметры функции: day={day}, is_Admin={is_Admin}, promo={promo}, help_message={help_message}, '
        ...
        # region Проверка, есть ли необходимый сервер
        yes = False
        for s in SERVERS:
        ...
        if not yes:
            ...
            return await send_admins(
                ...
                f'🛑Для протокола <b>{protocol}</b> нет серверов! '
                f'Необходимо после добавления сервера /add_server связаться с клиентом!'
            )
        # endregion
        ...
        if len(SERVERS) > 1:
            ...
            if ip_server is None:
                ...
                # Marzban только НЕ премиальный (и только для vless)
                if any(ser['is_marzban'] for ser in SERVERS) and protocol == 'vless':
                    logger.debug(f'{user_id} - есть сервер marzban и протокол vless, пробуем НЕ премиальный Marzban')
                    for ser in [s for s in SERVERS if s['is_marzban'] and not s['isPremium']]:
                        if await DB.get_count_keys_by_ip(ser['ip']) < ser['count_keys']:
                            server = ser
                            logger.debug(f'{user_id} - VLESS: выбрали НЕ премиальный Marzban со свободным местом, server={server}')
                            break
                ...
                if not server:
                    ...
                    while True:
                        ...
                        if not server_temp['is_pptp'] and protocol == 'pptp':
                            continue
                        if OSN_SERVER_NIDERLANDS and 'нидерланды' not in server_temp['location'].lower():
                        ...
                        if count_select_server >= 15:
                            logger.debug(f'{user_id} - Перебрали 15 рандомных серверов, пробуем выбрать по порядку (не прем)')
                            for ser in SERVERS:
                                if OSN_SERVER_NIDERLANDS and 'нидерланды' not in ser['location'].lower():
                                    continue
                            ...
                            logger.debug(f'{user_id} - Перебрали сервера по порядку, пробуем еще раз среди всех НЕ премиальных...')
                            for ser in SERVERS:
                                count_users_in_server = await DB.get_count_keys_by_ip(ser['ip'])
                                if count_users_in_server < ser['count_keys'] and not ser['isPremium']:
                                    server = ser
                                    logger.debug(f'{user_id} - Фолбэк: выбираем сервер по порядку среди НЕ премиальных, server={server}')
                                    break
                            break
            else:
        ...
        if server is None:
            ...
            for ser in SERVERS:
                ...
                if count_users_in_server < ser['count_keys']:
                    server = ser
                    logger.warning(f'{user_id} - Сервер не выбран, берем первый свободный (в т.ч. премиум) server={server}')
                    break
        ...
        if count_by_ip >= server['count_keys'] or count_by_ip >= 99999:
            logger.warning(f'{user_id} - Кол-во ключей на сервере больше чем count_keys или > 99999, пробуем создать на другом сервере')
            for ser in SERVERS:
                count_users_in_server = await DB.get_count_keys_by_ip(ser['ip'])
                if count_users_in_server < ser['count_keys']:
                    server = ser
        ...
        if count_keys >= count_keys_limit_percent:
            await send_admins(
                user_id,
                f'🛑Сервера заняты на >= 90%',
                f'⚠️Кол-во ключей на серверах: <b>{count_keys}</b>/{count_keys_limit}'
            )

        if count_by_ip >= server['count_keys'] or count_by_ip >= 99999:
            logger.warning(f'{user_id} - Кол-во ключей на сервере больше чем count_keys или > 99999')
            url = f'http://{server["ip"]}:51821'
            text_ip = f'<a href="{url}">{server["ip"]}</a>'
            await send_admins(
                user_id,
                f'🛑Не удалось создать ключ',
                f'<b>🛑Закончилось место на серверах!</b>\nЗаметка: {text_ip} (<b>{count_by_ip}</b> > {server["count_keys"]})'
            )
                ...
                if check_:
                    if protocol == 'wireguard':
                        ...
                        if check_server_is_marzban(server['ip']):
                            ...
                            # server['isPremium'] уже есть в структуре сервера,
                            # используем его как флаг премиальности локации
        ...
        if not text:
            ...
            await send_admins(
                user_id,
                f'🛑Не удалось создать ключ',
                f'🛑Не удалось обратиться к серверам, последний: <b>{server["ip"]}</b>\n'
                f'Заметка: <b>Ключ на {day} {await dney(day)}</b>{error}'
            )
            await send_message(user_id, user.lang.get('tx_no_create_key'))
            ...
            if SEND_QR:
                ...
                try:
                    ...
                    if result_qr:
                        ...
                        logger.warning(f'{user_id} - Wireguard создание QR не удалось (нет верного LOGO.png) result_qr={result_qr}')
                except:
                    logger.warning(f'{user_id} - Wireguard создание QR не удалось (нет верного LOGO.png) result_qr={result_qr}')
        ...
        if not silent:
            if is_Admin == 1:
                ...
                logger.debug(f'{user_id} - Отправка сообщения: админский ключ')
            else:
                try:
                ...
                promo_text = f'Ключ: <code>{conf_name}</code> ({date_key_str}, {day} {await dney(day)}, <code>{ip_server_created}</code>)'
                promo_text += f'\nПромокод: <b>{promo}</b>\n' if promo else ''
                ...
                if isChangeProtocol:
                    if not isChangeLocation:
                        ...
                        head_text = f'🔁Сменил локацию на 📍 <code>{ip_server_created}</code>'
                    ...
                    head_text = 'Выдал пробный 🔑'
                    await DB.add_otchet('get_test_keys')
                else:
                    head_text = 'Выдал 🆕 ключ'
...
# Простая проверка подписки на паблик-канал
async def is_user_subscribed(user_id: int, channel: str = "@evovpn_news") -> bool:
    try:
        ...
        # если канал приватный / бот не админ / юзера не видно — считаем, что не подписан
        return False
    ...
    try:
        if TEST_KEY:
            ...
            if not isGetTestKey:
                ...
                kb.add(InlineKeyboardButton(text="Подписаться", url="https://t.me/evovpn_news"))
                kb.add(InlineKeyboardButton(text="✅ Я подписался»", callback_data="trial:check"))
                await send_message(user_id, "Чтобы получить бесплатный тест-ключ, подпишитесь на наш канал. Это поможет нам держать вас в курсе обновлений и акций. После подписки нажмите «✅ Я подписался».", reply_markup=kb)
                return  # ждём нажатия «Проверить подписку»
            else:
...
async def trial_check_sub(call):
    try:
        ...
        if await is_user_subscribed(user_id, "@evovpn_news"):
            # (необязательно) удалим сообщение с кнопками
            try:
                await delete_message(user_id, call.message.message_id)
            except:
                pass

            # Запускаем стандартный флоу тестового периода
            user = await user_get(user_id)
            user.bot_status = 24
            ...
            await bot.answer_callback_query(
                ...
                show_alert=True,
                text="Пока не видим подписку. Это нужно только один раз, чтобы вы получали актуальные обновления. Подпишитесь и снова нажмите «✅ Я подписался»."
            )
    except:
        await Print_Error()
...
async def reset_all_premium_marzban_limits():
    try:
        ...
        logger.info(f'✅ reset_all_premium_marzban_limits: обновлено {updated} ключей')
        return updated
...
async def start_message(message):
    try:
        ...
        user_id = message.chat.id
        # Лучше вырезать аргумент корректно, даже если /start без пробела
        m_text = message.text.replace('/start', '', 1).strip()
        m_text_lower = m_text.lower().replace(' ', '')
        priglacili = False
        payload = (message.get_args() or '').strip()
        logger.debug(f'{user_id} - Нажал /start (args="{m_text}")')
        ...
        # --- share/code идут ПОСЛЕ ref/global и не запускаются при их наличии ---
        if 'global_' not in m_text and 'ref' not in m_text:
            # deep-link: https://t.me/Evo_vpn_bot?start=buy -> эмулируем нажатие кнопки "Подключить"
            if m_text_lower == 'buy' or (payload and payload.strip().lower() == 'buy'):
                user = await user_get(user_id)  # подтянуть язык пользователя
            ...
            if m:
                ...
                if promo:
                    ...
                    await send_message(user_id, "❗ Промокод пуст. Добавьте текст после 'code'.")
                    return
...
async def create_300_keys_message(message):
    """
    Тестовая команда: создать 300 ключей для проверки производительности.
    Ключи создаются как "истекающие сегодня" — при /test_update они должны отключиться.
    """
    try:
        user = await user_get(message.chat.id)
        if not user.isAdmin:
        ...
        # Получаем первый Marzban сервер
        marzban_server = None
        for server in SERVERS:
        ...
        if not marzban_server:
            await send_message(message.chat.id, '🛑 Не найден ни один Marzban сервер!')
            return
        
        marzban = MARZBAN(marzban_server['ip'], marzban_server['password'])
        
        # Создаём 300 ключей, которые "истекают сегодня" для теста отключения
        await marzban.create_test_keys_bulk(
            count=300,
            days=30,              # "Куплены" на 30 дней
            user_id=message.chat.id,
            ip_server=marzban_server['ip'],
            expire_today=True     # ← Ключи истекают сегодня (для теста /test_update)
        )
    except:
        await Print_Error()
...
async def cmd_test_reset(message):
    try:
        ...
        await message.answer("Запускаю тестовый сброс лимитов премиум-ключей...")
        updated = await reset_all_premium_marzban_limits()
        await message.answer(f"Готово. Обновлено ключей: {updated}")
    except:
        await Print_Error()
...
async def cmd_test_twohour(message):
    try:
        ...
        # здесь просто передаём "сейчас" как будущий момент отключения
        await send_twohour_notifications(datetime.now())

        await send_message(user_id, "send_twohour_notifications() выполнена")
    except:
        await Print_Error()
...
async def add_server_message(message):
    try:
        ...
        if user.isAdmin:
            ...
            text_primer = (
                f'🏷️Пример команды:\n\n/add_server <b>1.1.1.1</b> <b>svf4sg43</b> <b>99999</b> <b>🇳🇱Нидерланды</b> - Автоматическая настройка и добавление нового сервера в бота,\n'
                '    где <b>1.1.1.1</b> - ip сервера,\n'
                '    <b>svf4sg43</b> - пароль от сервера,\n'
                '    <b>240</b> - желаемое кол-во ключей на сервере (от 1 до 99999),\n'
...
async def create_promo_message(message):
    try:
        ...
        if user.isAdmin:
            ...
            primer = (
                f'🏷️Пример команды:\n\n'
                '/create <b>30%</b> <b>PROMO</b> <b>912863996</b> <b>50%</b> - Создание спец. ссылки для партнера, где\n'
                '    <b>30%</b> - скидка для клиентов на первую покупку,\n'
                '    <b>PROMO</b> - название для ссылки,\n'
                '    <b>912863996</b> - id партнера (для кого создаем ссылку),\n'
...
async def code_message(message):
    try:
        ...
        primer = (
            ...
            '    <b>7</b> - кол-во дней, которые он даёт клиенту,\n'
            '    <b>100</b> - возможное кол-во активаций,\n'
            '    <b>14</b> - кол-во дней, через которое удалится промокод'
        )

        try:
            code = m_text_sp[0]             # название промокода, например: evo10
            days = int(m_text_sp[1])        # сколько дней даёт
            count = int(m_text_sp[2])       # сколько раз можно активировать
            count_days_delete = int(m_text_sp[3])  # срок жизни промокода
        except:
            return await send_message(user_id, primer)
        ...
        if result:
            ...
            kb.add(
                InlineKeyboardButton(text="🔗 Активировать промокод", url=activate_link),
                InlineKeyboardButton(text="📣 Поделиться", url=share_link),
            )

            text = (
                f'✅ Индивидуальный промокод <b>{code}</b> создан!\n'
                f'📅 Дней по промокоду: <b>{days}</b>\n'
                f'🔢 Доступных активаций: <b>{count}</b>\n'
                f'⏳ Срок жизни (дней): <b>{count_days_delete}</b>\n\n'
            ...
            await send_message(
                user_id,
                f'🛑 Индивидуальный промокод <b>{code}</b> не удалось создать — он уже существует.'
            )
    except:
        await Print_Error()
            ...
            if keys_user and len(keys_user) > 0:
                ...
                if PODPISKA_MODE:
                    ...
                    for item in keys_user:
                        ...
                        name_key_for_but = f'Продлить 🔑{name_vpn_key} | Осталось {count_days_to_off_text}'
...
async def newpromo_message(message):
    try:
        ...
        if user.isAdmin:
            ...
            primer = '🏷️Пример команды: /newpromo <b>912863996</b> <b>500</b> <b>30</b> - Массовое создание промокодов с текстом (допустим для продажи в магазинах ключей), где\n    <b>912863996</b> - id партнера,\n    <b>500</b> - кол-во ключей,\n    <b>30</b> - на какое кол-во дней\n\n'
...
async def fun_klava_news_select(user=None, count_users=-1):
    try:
        ...
        for s in servers_list:
            ...
            mark = '✅' if user.news_select_servers.get(ip, False) else '⭕️'
        ...
        klava.add(InlineKeyboardButton(text=f'🗑️Отменить публикацию', callback_data=f'news_select:delete'))
        klava.add(InlineKeyboardButton(text=f'📣Опубликовать ({count_users})', callback_data=f'news_select:publish'))
        return klava
    except:
        await Print_Error()
        ...
        if user.isAdmin:
            ...
            if 'news_filter' in m_text and (temp_news_text != '' or user.news_photo_path != ''):
                ...
                text_send = (
                    ...
                    '⚠️При активации нескольких фильтров, будут выбраны пользователи, '
                    'которые подходят хотя бы под один фильтр!\n\n'
                )
                ...
                # DEBUG: вывод в лог, чтобы проверить сервера
                print("DEBUG buckets servers:", {k: len(v) for k, v in buckets.items() if k.startswith("srv:")})

                # 5) убрать «подсчет клиентов…» и показать меню
                try:
                    await delete_message(message.chat.id, mes_del.message_id)
                except:
                    pass
                ...
                if temp_news_text != '':
                    ...
                    if user.news_photo_path != '':
                        try:
                            ...
                            await send_message(
                                message.chat.id,
                                f'🛑Не удалось отправить новость!\n\n⚠️Ошибка:\n{e}'
                            )
                    elif temp_news_text != '':
            ...
            # ===== /news без текста =====
                ...
                primer = (
                    ...
                    '<but>Название переменной кнопки</but> (например but_main)\n\n'
...
async def handle_document(message):
    try:
        ...
        if user.isAdmin:
            ...
            if isConfigFile:
                await send_message(message.chat.id, f'✅Файл .env обновлен!')
            elif isRuFile:
    ...
    try:
        ...
        if is_paid:
            ...
            if user.isPayChangeProtocol:
                ...
                # Оплата за сброс трафика прошла
                user.isPayTraffic = False
                try:
                    user_id = user.id_Telegram
                    ...
                    if not server or not server.get('is_marzban') or not server.get('isPremium', False):
                        await send_message(
                            user_id,
                            '⚠️Сброс трафика возможен только для премиальных Marzban-локаций.'
                        )
                    else:
                        marzban = MARZBAN(ip=server['ip'], password=server['password'])

                        # Сбрасываем лимит и использованный трафик
                        ...
                        try:
                            await DB.add_operation(
                                ...
                                f'Сброс трафика (лимит 500 ГБ, месяц) для {vpn_key}'
                            )
                        except:
                            pass
                        ...
                        await send_message(
                            user_id,
                            f'✅Для ключа <code>{vpn_key}</code> выполнен сброс лимита и использованного трафика.\n'
                            f'Новый лимит: 500 ГБ, сброс — раз в месяц.'
                        )
                except Exception as e:
                    logger.warning(f'🛑Ошибка при сбросе трафика после оплаты: {e}')
                    await Print_Error()
                finally:
                    user.bill_vpn_key = ''
                ...
                if is_key_exists_in_db:
                    # Продление существующего ключа — без сообщения пользователю
                    vpn_key = user.bill_vpn_key
                    user.bill_vpn_key = ''
                    ...
                    if not IS_OTCHET:
                        await send_admins(
                            ...
                            f'(<b>+{summ}₽</b>, +{days_plus} {await dney(days_plus)}, <code>{ip_server}</code>){bottom_text}'
                        )
                    await DB.add_otchet('prodleny')
                else:
                    # Выдача нового ключа — отправляем сообщение пользователю
                    ...
                    if not IS_OTCHET:
                        await send_admins(
                            user_id,
                            '✅Оплата ключа',
                            f'Ключ: <code>{vpn_key or "—"}</code> '
                            f'(<b>+{summ}₽</b>, {days_plus} {await dney(days_plus)}{ip_part}){bottom_text}'
                        )
                    await DB.add_otchet('get_new_keys')

                    # Уведомление пользователю — ТОЛЬКО при новом ключе
                    user = await user_get(user_id)
        ...
        if user.isAdmin:
            ...
            if is_search and (not data) and search_text:
                ...
                if key_row:
                    ...
                    temp_user_id = key_user_id  # <-- важно, чтобы ниже не было None
                    data = await DB.get_all_users_report(str(key_user_id), True)
                else:
                    await send_message(user_id, '⚠️Не найдено ни пользователя с таким ID, ни QR-ключа.')
                    await delete_message(user_id, mes_del.message_id)
                    return
...
async def news_select(call):
    try:
        ...
        if 'delete' in call.data:
            ...
            # Если выбраны все три ОС — используем полный охват, как и раньше
            ...
            if user.users_ids == []:
                await bot.answer_callback_query(
                    callback_query_id=call.id,
                    show_alert=True,
                    text="⚠️Нет пользователей, подходящих по фильтру, для отправки новости!"
                )
                return
        ...
        # ===== Пересчёт аудитории при изменении фильтров =====
        if isUpdateFilter:
            # Если выбраны все ОС — берём полный охват (как в исходной логике)
            if user.news_select_android and user.news_select_ios and user.news_select_windows:
                all_ids = []
                for arr in user.users_ids_news_select.values():
                    all_ids.extend(arr)
            ...
            # Обновляем клавиатуру и счётчик
            try:
                ...
                await bot.answer_callback_query(callback_query_id=call.id, show_alert=False, text="✅Фильтр обновлён")
            except:
                await Print_Error()
...
async def servers_edit(call=None, ip=None, message=None):
    try:
        ...
        # Жёсткая защита: только админ может управлять серверами
        if not user.isAdmin:
            # Можно просто молча выйти или отправить сообщение
            await send_message(user_id, '🛑 ИДЕШЬ НАХУЙ.')
            return
        ...
        if server:
            if call and 'reboot' in call.data:
                ...
                text = (
                    f'ℹ️Введите максимальное кол-во ключей для сервера <b>{ip}</b> <b>(от 1 до 99999)</b>:\n'
...
async def add_wallet_call(call):
    try:
        ...
        if call.data == 'add_wallet:':
            ...
            await send_message(user_id, instruction, reply_markup=klava)
            if PHONE_NUMBER == '':
                await send_message(user_id, "🛑Для того, чтобы пользователи могли оплачивать через Tinkoff Pay, необходимо указать номер телефона в настройках бота (.env -> PHONE_NUMBER = '79999999999')!")
                return
            user.bot_status = 45
...
async def ban_user_call(call):
    try:
        ...
        if user_ban == MY_ID_TELEG:
            ...
            return await send_message(MY_ID_TELEG, f'🛑🛑🛑Пользователь <b>{user_id}</b> попытался заблокировать тебя!🛑🛑🛑')
...
async def change_app_call(call: types.CallbackQuery):
    ...
    if len(keys_data) == 0:
        await call.message.edit_text("Не найдено активных ключей!")
        return

    if len(keys_data) > 1:
        klava = InlineKeyboardMarkup()
        for key in keys_data:
            klava.add(InlineKeyboardButton(text=f"Ключ {key[0]}", callback_data=f"change_app_key:{key[0]}"))
        await call.message.edit_text("Выберите ключ:", reply_markup=klava)
    else:
...
async def show_platforms(call: types.CallbackQuery, vpn_key: str):
    ...
    await call.message.edit_text("Выберите платформу:", reply_markup=klava)
...
async def show_apps(call: types.CallbackQuery, vpn_key: str, platform: str):
    ...
    await call.message.edit_text("Выберите приложение:", reply_markup=klava)
...
async def final_app_selection(call: types.CallbackQuery):
    ...
    if not key_data:
        await call.message.edit_text("Ошибка: ключ не найден!")
        return
    ...
    if '#' in key_url:
        ...
        instruction_text = (
            "1. Скачайте и установите приложение, перейдя по кнопке \"Скачать приложение\"\n\n"
            "2. Добавьте ключ нажатием на кнопку ниже 👇"
        )

    klava = InlineKeyboardMarkup()
    klava.add(InlineKeyboardButton(text="1. Скачать приложение", url=app_links[app][platform]))
    klava.add(InlineKeyboardButton(text="2. Добавить ключ", url=import_link))
    ...
    try:
        ...
        if 'ch_loc' in call_data:
            ...
            if not isSelect:
               ...
               # берём текущую локацию и IP сервера ключа
                old_location = ''
                old_ip_server = ''
                old_key_date = None
                ...
                # --- ОГРАНИЧЕНИЕ ДЛЯ ПРЕМИУМ-ЛОКАЦИЙ: СМЕНА 1 РАЗ В 30 ДНЕЙ ---

                # собираем список премиум-IP
                try:
                    premium_ips = {server['ip'] for server in SERVERS if server.get('isPremium')}
                except Exception as e:
                    logger.warning(f'⚠️Не удалось собрать premium_ips: {e}')
                    premium_ips = set()

                is_current_premium = old_ip_server in premium_ips

                if is_current_premium:
                    # по умолчанию считаем, что на премиум попал в дату создания ключа
                    last_premium_change = old_key_date

                    # ищем последнюю операцию смены локации на премиальную
                    user_operations_data = await DB.get_user_operations(user_id=user_id)
                    for operation in user_operations_data:
                        if operation[1] == 'change_location':
                            desc = operation[6] or ''
                    ...
                    if last_premium_change:
                        ...
                        if (now - last_premium_change) <= timedelta(days=999):
                            await send_message(
                                user_id,
                                '❗️У вас активна мульти-локация, включающая все локации и обходы LTE.'
                            )
                            return

                # --- конец ограничения для премиум-локаций ---            
                klava = InlineKeyboardMarkup()
        ...
        if 'prodlit' in call_data:
            logger.debug(f'{user_id} - Ключ {vpn_key} выбрал продление')
            await delete_message(user_send, message.message_id)
            user.isProdleniye = vpn_key

            # Проверяем – премиальная ли локация на Marzban
            ...
            try:
                ...
                if ip_server:
                    for server in SERVERS:
                        if server['ip'] == ip_server:
                            # is_marzban и isPremium должны быть в описании сервера
                            if server.get('is_marzban') and server.get('isPremium', False):
                                is_premium_location = True
                            break
            except Exception as e:
                logger.warning(f'🛑Ошибка при проверке премиальной локации для {vpn_key}: {e}')

            # Базовая клавиатура продления (копируем user.klav_buy_days)
            ...
            try:
                ...
                logger.warning(f'🛑Ошибка копирования klav_buy_days: {e}')
                # если вдруг не удалось скопировать – используем оригинал
                klava = user.klav_buy_days

            # Если ключ на премиальной Marzban-локации – добавляем кнопку сбросить ТРАФИК
            ...
            # Текст такой же, как и был – просто с нашей обновлённой клавиатурой
            await send_message(
                user_id,
...
async def process_create_key(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton(text='🙅‍♂️Отменить ввод', callback_data='cancel_key_days'))

    await call.message.answer('Введите количество дней для нового ключа:', reply_markup=kb)
    await call.answer()
...
async def process_key_days(message: Message):
    ...
    if not txt.isdigit():
        await message.reply('Введите правильное число дней (1–1000)!')
        return

    days = int(txt)

    if days < 1:
        await message.reply('Количество дней должно быть минимум 1.')
        return

    if days > 1000:
        await message.reply('Лимит: максимум 1000 дней. Введите число от 1 до 1000.')
        return

    user_id_rep = admin_user.temp_create_key_user_id

    # Создаём ключ (только VLESS, без админских прав)
    ...
    await message.reply(f'✅ Ключ для пользователя {user_id_rep} успешно создан на {days} дней.')
...
async def cancel_key_days(call: CallbackQuery):
    ...
    try:
        await call.message.edit_text('❌ Ввод отменён.')
    except:
...
async def buy_traffic_call(call):
    try:
        ...
        # Определяем сервер и проверяем премиум + Marzban
        ip_server = await DB.get_ip_server_by_key_name(vpn_key)
        server = None
        for s in SERVERS:
            if s['ip'] == ip_server:
        ...
        if not server or not server.get('is_marzban') or not server.get('isPremium', False):
            await bot.answer_callback_query(
                callback_query_id=call.id,
                text='Сброс трафика доступен только для премиальных Marzban-локаций',
                show_alert=True
            )
            return

        # Готовим оплату за трафик
        ...
        # Если несколько кошельков – сначала выберем способ оплаты
        if len([item for item in WALLETS if item['isActive']]) > 1 and not user.PAY_WALLET:
            user.payStatus = 1
            await select_payment_method(user_id)
            return

        # Запускаем общую процедуру покупки
        await pokupka(user)
    except Exception as e:
        logger.warning(f'🛑Ошибка в buy_traffic_call: {e}')
        await Print_Error()
...
async def tv_back_call(call):
    ...
    try:
        # Возвращаем исходный вид: inline-кнопки выбора устройства (как после выдачи ключа)
        await call.message.edit_text(
            text_back,
            reply_markup=await fun_klav_podkl_no_back(user, user.buttons_podkl_vless)
        )
    except Exception:
        # Если не можем отредактировать (например, сообщение слишком старое) — просто отправим заново
        await send_message(
            user_id,
            text_back,
            reply_markup=await fun_klav_podkl_no_back(user, user.buttons_podkl_vless)
...
async def buttons_call(call):
    try:
        ...
        # --- ЧИСТИМ ЧАТ: удаляем сообщение с кнопками и ещё 1 предыдущее ---
...
async def pokupka(user):
    try:
        ...
        # это "старая цена" для зачёркнутого отображения (может быть пустой строкой)
        tarif_else_1_tarif = ''

        # ==== Обычная покупка / продление ====
        if (
        ...
        # ==== Специальные платежи (смена протокола / локаций / сброс трафика) ====
        else:
            if user.isPayChangeProtocol:
                tarif_select = 11
                ...
                # для докупки трафика "старую цену" не показываем
                tarif_else_1_tarif = ''
        ...
        # ==== Сообщение пользователю с кнопкой "Оплатить" ====
        if not is_xtr:
            klava_buy = InlineKeyboardMarkup()
            if not is_rootpay:
                klava_buy.add(
        ...
        # Дополнительная подсказка по переводу
        if not is_rootpay:
            if PHONE_NUMBER != '':
                await send_message(
                    user_id,
        ...
        if user.isPayChangeProtocol:
            ...
            bottom_text = (
                f'🆔Bill_id: <code>{user.bill_id.split("-")[-1]}</code> '
                f'(<b>{tarif}</b>₽)'
            )
        elif user.isPayChangeLocations:
            ...
            # лог по СБРОСУ трафика
            head_text = f'📶Вызвал оплату сброса трафика'
            text_add = ''
            if user.bill_vpn_key:
                text_add = f', ключ <code>{user.bill_vpn_key}</code>'
            bottom_text = (
                f'🆔Bill_id: <code>{user.bill_id.split("-")[-1]}</code> '
                f'(<b>{tarif}</b>₽{text_add})'
            )
        else:
            # обычная покупка / продление
            if user.bill_vpn_key != '':
                ip_server = await DB.get_ip_server_by_key_name(user.bill_vpn_key)
                head_text = '☑️Вызвал продление ключа'
                text_add = f', <code>{user.bill_vpn_key}</code>'
                bottom_text = (
                    f'🆔Bill_id: <code>{user.bill_id.split("-")[-1]}</code> '
                    f'(<b>{tarif}</b>₽{text_add}, <code>{ip_server}</code>)'
                )
            else:
                head_text = '☑️Вызвал оплату ключа'
                bottom_text = (
                    f'🆔Bill_id: <code>{user.bill_id.split("-")[-1]}</code> '
                    f'(<b>{tarif}</b>₽)'
                )
...
async def help(user_id, id, protocol=PR_DEFAULT):
    try:
        ...
        # ===== Тексты кнопок (с фолбэками для совместимости со старым lang.yml) =====
        tx_download_generic = user.lang.get('instr_2_0_download')

        # Отдельные названия кнопок по платформам/сторам (если ключа нет — берём старое общее название)
        ...
        # В тексте инструкции используем плейсхолдеры {tx_download} и {tx_install}.
        ...
        # Разные тексты (подвала) под платформы: Android / iOS / macOS / Windows
        ...
        if user.key_url:
            ...
            if protocol == 'outline':
                ...
                if id == 1:  # Android
                    ...
                    url += 'happ://add/' + user.key_url.replace('&', 'a_n_d')
                    # RU App Store + Global App Store (App доступен и для Mac)
...
async def message_input(message, alt_text=''):
    try:
        ...
        if user.bot_status == 1: # Просмотр определенного пользователя
            ...
            if user.keyForChange != '':
                if 1 <= index <= 99999:
                    ...
                    await send_message(message.chat.id, f'🛑Максимальное кол-во ключей для сервера <b>от 1 до 99999</b>!')
            else:
        ...
        elif user.bot_status == 90:  # Happ Android TV: ждём UID (5 символов) для Web Import
            uid_raw = (message.text or '').strip()
            uid = uid_raw.upper()

            # Таймаут ожидания UID (90 секунд). Если пользователь долго не присылал код — сбрасываем режим.
            ts = getattr(user, 'tv_uid_requested_at', 0)
            if ts and (time.time() - ts > 90):
                user.bot_status = 0
                user.tv_uid_requested_at = 0
            ...
            # Отмена ожидания UID (пользователь может нажать "Назад" или написать "Назад")
            if uid in ('⏪ НАЗАД', 'НАЗАД', '/CANCEL', '/START', '/MENU'):
                user.bot_status = 0
                user.tv_uid_requested_at = 0
            ...
            # UID на экране ТВ: обычно 5 символов (A-Z/0-9)
            if not re.match(r'^[A-Z0-9]{5}$', uid):
                await send_message(user_id, '❌ UID должен быть из 5 символов (A-Z, 0-9). Пришли код с экрана ТВ ещё раз или нажми ⏪ Назад.')
                return

            # Нужен уже выданный ключ
            if not getattr(user, 'key_url', ''):
                user.bot_status = 0
                user.tv_uid_requested_at = 0
                await send_message(user_id, '❌ Не нашёл у тебя активный ключ. Сначала получи/создай ключ, потом снова нажми Android TV.')
                return
            ...
            try:
                ...
                if r.status_code // 100 != 2:
                    await send_message(user_id, f'⚠️ Happ вернул ошибку (HTTP {r.status_code}). Проверь UID и попробуй ещё раз.')
                    return

                # 2) Некоторые ответы могут приходить 200 OK, но с текстом ошибки внутри
                body_text = (r.text or '').strip()
                body_low = body_text.lower()

                # Типичные тексты ошибок (например: "Пользователи по данному запросу не найдены!")
                if ('не найд' in body_low) or ('not found' in body_low):
                    await send_message(user_id, '⚠️ UID не найден в Happ. Проверь код на ТВ и пришли ещё раз.')
                    return

                # 3) Если ответ JSON — проверяем флаги ошибки/успеха, если они есть
                ok = True
                try:
                    js = r.json()
                    if isinstance(js, dict):
                        ...
                        if ('не найд' in msg) or ('not found' in msg):
                            ok = False
                except:
                    pass
                ...
                await send_message(
                    user_id,
                    '✅ Запрос отправлен в Happ.\n\nЕсли UID был актуальный и на ТВ открыт Web Import — подписка появится в приложении. Если не появилось, пришли UID ещё раз или нажми ⏪ Назад.',
                    reply_markup=klava_tv_back
                )

            except:
                await Print_Error()
                await send_message(user_id, '⚠️ Не смог отправить подписку в Happ (ошибка сети). Попробуй ещё раз.')
                return
            ...
            # Кнопка "Назад" — отменяет ожидание UID
            klava_tv_back = InlineKeyboardMarkup()
            klava_tv_back.add(InlineKeyboardButton(text='⏪ Назад', callback_data='tv:back'))
            ...
            await send_message(
                user_id,
                "📺 <b>Android TV (Happ)</b>\n\n"
                "1) Установи и открой Happ на Android TV\n"
                "2) Перейди в <b>Поделиться через Веб</b>\n"
                "3) Отправь мне код из 5 символов, который видишь на экране ТВ\n\n"
                "После этого я сам добавлю подписку на ТВ.",
                reply_markup=klava_tv_back
            )
            ...
            if '.' in m_text:
                for server in SERVERS:
                    if server['ip'] == m_text:
                        # Если не админ — сразу шлём отказ и выходим
                        if not user.isAdmin:
                            await send_message(user_id, '🖕 ИДЕШЬ НА ХУЙ.')
                            return

                        # Если админ — открываем меню управления сервером
